...
void loop() {
  ...
  while (pulseSensor.sawStartOfBeat()) {
    ...
    float salineNeeded = map(heartRate, 60, 120, 100, 500); // Map heart rate to saline needed (60-120 BPM to 100-500 ml)
...
void waterFlow(int value) {
  ...
  if (currentTime - oldTime > 1000) { // Update every 1 second
   //totalMilliLitres=0;
    ...
    flowMilliLitres =  ((pulseCount *1000) / (7.5*60)); // Convert flow rate to milliliters per second
